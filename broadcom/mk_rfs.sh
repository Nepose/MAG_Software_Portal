...
# $4 - LEB size
# $5 - PAGE size
    ./or_mkfs.ubifs -r $1 -m $5 -e $4 -c $3 -o $2
}
...
case $ROOTFS_FS_TYPE in
	...
	page_size=2048
	max_lebs=1710
	if [ "$ROOTFS_FS_UBI_LEBS" != "" ] ; then
	    max_lebs=$ROOTFS_FS_UBI_LEBS
	fi
	if [ "$ROOTFS_FS_UBI_LEB_SIZE" != "" ] ; then
	    leb_size=$ROOTFS_FS_UBI_LEB_SIZE
	fi
	if [ "$ROOTFS_FS_UBI_PAGE" != "" ] ; then
	    page_size=$ROOTFS_FS_UBI_PAGE
	fi

	make_image_ubi ./tmp/rootfs ./sumsubfsnone.img $max_lebs $leb_size $page_size
    ;;
    ext4)
	./mk_rfs_ext4.sh ./tmp/rootfs ./sumsubfsnone.img
    ;;
    jffs2)
