	...
	this.bpanel.Add(KEYS.OK, 'ok.png', _('Create'), function () {
		...
		function checkSize () {
			ajax('HEAD', url, function ( response, status, xhr ) {
				// check if our modal still exist (user can close it or create new request before we get answer from server)
		...
		if ( url && validateUrl(url) ) {
			...
			if ( !name ) {
				return new CModalAlert(self, _('Wrong data'), _('Provided name is invalid'), _('Close'));
			}

			if ( self.device.GetValue() === -1 ) {
				return new CModalHint(self, _('No storage device found'), 3000);
			}
